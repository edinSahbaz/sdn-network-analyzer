...
def main():
    ...
    while True:
        ...
        if eth_proto == 'IPV6':
            newPacket, nextProto = ip_v6_header(data, filter)
...
def print_packets_v6(filter, nextProto, newPacket):
    remainingPacket = ""

    if nextProto == 'ICMPv6' and (len(filter) == 0 or filter[2] == "ICMPv6"):
        remainingPacket = icmp_v6_header(newPacket)
    elif nextProto == 'TCP' and (len(filter) == 0 or filter[2] == "TCP"):
        remainingPacket = tcp_header(newPacket)
    elif nextProto == 'UDP' and (len(filter) == 0 or filter[2] == "UDP"):
        remainingPacket = udp_header(newPacket)

    return remainingPacket


def tcp_header(newPacket):
